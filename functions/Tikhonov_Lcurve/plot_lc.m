...
xlabel('Residual norm $$ ||\mathbf{G}_{sv}\mathbf{q}_{(\lambda)}^p - \mathbf{v}_{s_n}^p || $$','Interpreter','latex')
if (ps==1)
  ylabel('Solution norm $$ ||\mathbf{q}_{(\lambda)}^p|| $$','Interpreter','latex')
else
  ylabel('Solution semi-norm $$ || \mathbf{\Gamma q}_{(\lambda)}^p || $$','Interpreter','latex')
end
