function [velocities] = DESM(frequencies, micSpectra, D_by_bc, D_frequencies_by_bc, r_mic, r_nodes, r_q, normData, physicsParameters)
...
for ii = 1:length(frequencies)
    ...
    [subD, ~, selectedGroups] = createSubdictionary(D_by_bc, D_frequencies_by_bc, freq, length(D_by_bc));
    ...
    nP = norm(P);
    Epsilon = nP * 50 / 100; % constraint: error rate
    gammaArray = spg_bpdn(H, P, Epsilon, options);
